...
import { ResponseAxios } from "../components/InputField/inputField";
import axios from 'axios';

export const hydrateList = createAsyncThunk('movie/hydrateList', async () => {

    const { data: { data } } = await axios.get('https://code-challenge.spectrumtoolbox.com/api/movies', {
        headers: {
...
export const initialState: initVal = {
    movieList: [],
    genres: [''],
    loading: false,
    searchList: [],
...
const movie = createSlice({
    ...
    reducers: {
        filterList: (state, action) => {
            ...
            state.searchList = state.movieList.filter(item => (action.payload === "" || item.genres.includes(action.payload))
                && item.title.toLocaleLowerCase().includes(state.keyword.toLocaleLowerCase()));
        },
        filterTitle: (state, action) => {
            console.log("title")
            state.keyword = action.payload;
            state.searchList = state.movieList.filter(item => (state.genre === "" || item.genres.includes(state.genre))
